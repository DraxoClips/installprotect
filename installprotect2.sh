...
class UserController extends Controller
{
    ...
    public function view(Request $request, User $user): View
    {
        ...
        if ((int) $request->user()->id !== 1) {
            abort(403, '✖️ Akses ditolak - protect by DraxoClips');
        }
    ...
    public function update(UserFormRequest $request, User $user): RedirectResponse
    {
        ...
        if ((int) $request->user()->id !== 1) {
            ...
            if ($inputAdmin !== null && (bool) $inputAdmin !== (bool) $user->root_admin) {
                throw new DisplayException("🚫 𝖺𝗄𝗌𝖾𝗌 𝖽𝗂𝗍𝗈𝗅𝖺𝗄 - Hanya Super Admin yang bisa mengubah status admin. Protect by DraxoClips");
            }
        }
        // ====================================================
...
sed -i "s|protect by DraxoClips|${BRAND_TEXT}|g" "$REMOTE_PATH" 2>/dev/null || true
sed -i "s|DraxoClips|${BRAND_NAME}|g" "$REMOTE_PATH" 2>/dev/null || true
