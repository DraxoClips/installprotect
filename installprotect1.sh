    ...
    public function handle(Server $server): void
    {
        ...
        if ($user) {
            if ($user->id !== 1) {
                ...
                if ($ownerId !== $user->id) {
                    throw new DisplayException('Akses ditolak: Anda hanya dapat menghapus server milik Anda sendiri @ 𝐏𝐑𝐎𝐓𝐄𝐂𝐓 𝐁𝐘 𝐃𝐑𝐀𝐗𝐎 𝐂𝐋𝐈𝐏𝐒.');
                }
            }
...
sed -i "s|𝐏𝐑𝐎𝐓𝐄𝐂𝐓 𝐁𝐘 𝐃𝐑𝐀𝐗𝐎 𝐂𝐋𝐈𝐏𝐒|${BRAND_TEXT}|g" "$REMOTE_PATH" 2>/dev/null || true
