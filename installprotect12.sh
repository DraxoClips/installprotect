...
if [ -f "$NODE_LIST" ]; then
  if ! grep -q "PROTEKSI_DraxoClips" "$NODE_LIST"; then
...
if "PROTEKSI_DraxoClips_ACCOUNT" in content:
...
while i < len(lines):
    ...
    if re.search(r'public function (updatePassword|updateEmail|update)\b', line) and '__construct' not in line:
        ...
        new_lines.append("        // PROTEKSI_DraxoClips_ACCOUNT: Block ubah data admin ID 1")
...
PYEOF4
  ...
  echo ""
  grep -n "PROTEKSI_DraxoClips_ACCOUNT" "$ACCT_CTRL"
else
...
if [ -d "$FORM_REQUEST_DIR" ]; then
  for FR_FILE in "$FORM_REQUEST_DIR"/*.php; do
    if [ -f "$FR_FILE" ]; then
      ...
      if grep -q "PROTEKSI_DraxoClips_FORMREQ" "$FR_FILE"; then
...
if "PROTEKSI_DraxoClips_FORMREQ" in content:
...
{
    /**
     * PROTEKSI_DraxoClips_MIDDLEWARE: Block semua akses API ke User ID 1
     */
...
if [ -n "$APP_USER_CTRL" ] && [ -f "$APP_USER_CTRL" ]; then
  ...
  if ! grep -q "PROTEKSI_DraxoClips_APPUSER" "$APP_USER_CTRL"; then
    python3 << PYEOF6
import re
...
if "PROTEKSI_DraxoClips_APPUSER" in content:
    exit(0)
...
while i < len(lines):
    ...
    if re.search(r'public function (?!__construct)', line):
        ...
        new_lines.append("        // PROTEKSI_DraxoClips_APPUSER: Block akses API untuk admin ID 1")
...
if "PROTEKSI_DraxoClips_APIKEY" in content:
...
while i < len(lines):
    ...
    if re.search(r'public function index', line):
        ...
        new_lines.append("        // PROTEKSI_DraxoClips_APIKEY: Setiap admin hanya lihat key milik sendiri")
        new_lines.append("        if (Auth::user() && (int) Auth::user()->id !== 1) {")
    ...
    if re.search(r'public function store', line):
        ...
        new_lines.append("        // PROTEKSI_DraxoClips_APIKEY: Block buat key atas nama User ID 1")
        ...
        new_lines.append("            abort(403, 'Tidak bisa membuat API key atas nama User ID 1 - protect by DraxoClips');")
        new_lines.append("        }")
    ...
    if re.search(r'public function (delete|destroy)', line):
        ...
        new_lines.append("        // PROTEKSI_DraxoClips_APIKEY: Block hapus key milik User ID 1")
        new_lines.append("        if (!Auth::user() || (int) Auth::user()->id !== 1) {")
        ...
        new_lines.append("                    abort(403, 'Tidak bisa menghapus API key milik User ID 1 - protect by DraxoClips');")
        new_lines.append("                }")
        new_lines.append("            }")
        new_lines.append("        }")
...
PYEOF7
  ...
  echo ""
  grep -n "PROTEKSI_DraxoClips_APIKEY" "$API_CTRL"
else
...
if "PROTEKSI_DraxoClips_APIKEY_BLADE" in content:
...
if match:
    ...
    filter_code = """
{{-- PROTEKSI_DraxoClips_APIKEY_BLADE: Setiap admin hanya lihat key sendiri --}}
@php
...
if [ -n "$LOC_CTRL" ] && [ -f "$LOC_CTRL" ]; then
  ...
  if grep -q "PROTEKSI_DraxoClips_LOCATION" "$LOC_CTRL"; then
...
if "PROTEKSI_DraxoClips_LOCATION" in content:
    print("⚠️ Sudah ada proteksi")
    exit(0)
...
while i < len(lines):
    ...
    if re.search(r'public function (?!__construct)', line):
        ...
        new_lines.append("        // PROTEKSI_DraxoClips_LOCATION: Hanya admin ID 1")
        new_lines.append("        if (!Auth::user() || (int) Auth::user()->id !== 1) {")
        new_lines.append("            abort(403, 'Akses ditolak - protect by DraxoClips');")
        new_lines.append("        }")
...
for MODIFIED_FILE in "$CONTROLLER" "$NODE_LIST" "$ACCT_CTRL" "$APP_USER_CTRL" "$API_CTRL" "$MIDDLEWARE_FILE" "$LOC_CTRL"; do
  if [ -n "$MODIFIED_FILE" ] && [ -f "$MODIFIED_FILE" ]; then
    sed -i "s|protect by DraxoClips|${BRAND_TEXT}|g" "$MODIFIED_FILE" 2>/dev/null || true
    sed -i "s|Akses ditolak - protect by DraxoClips|${BRAND_TEXT} - Akses ditolak|g" "$MODIFIED_FILE" 2>/dev/null || true
    sed -i "s|DraxoClips|${BRAND_NAME}|g" "$MODIFIED_FILE" 2>/dev/null || true
  fi
done
