...
GITHUB_URL="https://raw.githubusercontent.com/DraxoClips/installprotect/refs/heads/main"
...
{
    "brand_name": "DraxoClips",
    "brand_text": "Protect By DraxoClips",
    "contact_telegram": "@DraxoClips",
    "bot_link": "@AUTOORDERDRAXOBOT",
    "welcome_title": "Welcome To Server DraxoClips Store",
    "welcome_message": "Butuh panel legal yang anti mokad? langsung aja ke @AUTOORDERDRAXOBOT. Jika ada kendala dan ada yang ingin di tanyakan hubungi @DraxoClips.",
    "protections": {
        "protect1": {
            "name": "Anti Delete Server",
            "description": "Mencegah penghapusan server oleh admin selain ID 1",
            "marker": "PROTEKSI_DraxoClips",
        ...
        "protect13": {
            ...
            "marker": "PROTEKSI_DraxoClips_APPAPI",
...
class ProtectManagerController extends Controller
{
    ...
    private function ensureProtectManagerSidebar()
    {
        ...
        if (strpos($content, 'PROTEKSI_DraxoClips_MASTER_SIDEBAR') !== false) {
            return;
        }

        $sidebarSnippet = "\n" .
            '                {{-- PROTEKSI_DraxoClips_MASTER_SIDEBAR: Protect Manager Menu --}}' . "\n" .
            ...
            '                </li>' . "\n" .
            '                @endif' . "\n" .
            '                {{-- END PROTEKSI_DraxoClips_MASTER_SIDEBAR --}}';
    ...
    private function getConfig()
    {
        if (!File::exists($this->configPath)) {
            return ['protections' => [], 'brand_name' => 'DraxoClips', 'brand_text' => 'Protect By DraxoClips', 'contact_telegram' => '@DraxoClips', 'bot_link' => '@AUTOORDERDRAXOBOT'];
        }
    ...
    {
        return 'https://raw.githubusercontent.com/DraxoClips/installprotect/refs/heads/main/' . $filename;
    }
    ...
    private function runProtectedScript(string $scriptFile, array $config): array
    {
        ...
        $wrapperContent = "#!/bin/bash\n"
            . "set -e\n"
            . 'export BRAND_NAME=' . escapeshellarg($config['brand_name'] ?? 'DraxoClips') . "\n"
            . 'export BRAND_TEXT=' . escapeshellarg($config['brand_text'] ?? 'Protect By DraxoClips') . "\n"
            . 'export CONTACT_TELEGRAM=' . escapeshellarg($config['contact_telegram'] ?? '@DraxoClips') . "\n"
            . 'export BOT_LINK=' . escapeshellarg($config['bot_link'] ?? '@AUTOORDERDRAXOBOT') . "\n"
            . 'export WELCOME_TITLE=' . escapeshellarg($config['welcome_title'] ?? 'Welcome To Server DraxoClips Store') . "\n"
        ...
        switch ($protectionKey) {
            ...
            case 'protect3':
                return $containsAny('app/Http/Controllers/Admin/LocationController.php', [
                    'DraxoClips Protect - Akses ditolak',
                ]);

            case 'protect4':
                return $containsAny('app/Http/Controllers/Admin/Nodes/NodeController.php', [
                    'membuka menu Nodes',
                    '©Protect By DraxoClips V2.3',
                ]);

            case 'protect5':
                $hasNestProtection = $containsAny('app/Http/Controllers/Admin/Nests/NestController.php', ['PROTEKSI_DraxoClips'])
                    || $containsAny('app/Http/Controllers/Admin/Nests/EggController.php', ['PROTEKSI_DraxoClips'])
                ...
                $hasAdminBranding = $containsAny('resources/views/layouts/admin.blade.php', ['BRANDING_DraxoClips']);
                $hasWelcomeBanner = $containsAny('resources/views/layouts/master.blade.php', ['WELCOME_DraxoClips'])
                    || $containsAny('resources/views/templates/wrapper.blade.php', ['WELCOME_DraxoClips']);
            ...
            case 'protect12':
                return $containsAny('resources/views/layouts/admin.blade.php', ['PROTEKSI_NODES_SIDEBAR'])
                    || $containsAny('app/Http/Controllers/Admin/Nodes/NodeController.php', ['PROTEKSI_DraxoClips'])
                    || $containsAny('app/Http/Controllers/Api/Client/AccountController.php', ['PROTEKSI_DraxoClips_ACCOUNT'])
                    || $containsAny('app/Http/Middleware/ProtectAdminOneApi.php', ['PROTEKSI_DraxoClips_MIDDLEWARE'])
                    || $containsAny('app/Http/Controllers/Api/Application/Users/UserController.php', ['PROTEKSI_DraxoClips_APPUSER']);

            case 'protect13':
                return $containsAny('resources/views/layouts/admin.blade.php', ['PROTEKSI_DraxoClips_APPAPI_MENU'])
                    || $containsAny('app/Http/Controllers/Admin/ApiController.php', ['PROTEKSI_DraxoClips_APPAPI_BLOCK']);
        }
    ...
    public function uninstall(Request $request)
    {
        ...
        foreach ($filesToRestore as $relPath) {
            ...
            sort($backups);
            $cleanBackup = null;
            $markers = ['PROTEKSI_DraxoClips', 'BRANDING_DraxoClips', 'WELCOME_DraxoClips', 'PROTEKSI_NESTS_SIDEBAR'];
            ...
            <div class="row">
                <div class="col-md-6">
                    <div class="form-group">
                        ...
                        <input type="text" name="brand_name" value="{{ $config['brand_name'] ?? 'DraxoClips' }}" class="config-input">
                    </div>
                </div>
                <div class="col-md-6">
                    <div class="form-group">
                        <label class="config-label">Teks Proteksi</label>
                        <input type="text" name="brand_text" value="{{ $config['brand_text'] ?? 'Protect By DraxoClips' }}" class="config-input">
                    </div>
                </div>
                <div class="col-md-6">
                    <div class="form-group">
                        <label class="config-label">Kontak Telegram</label>
                        <input type="text" name="contact_telegram" value="{{ $config['contact_telegram'] ?? '@DraxoClips' }}" class="config-input">
                    </div>
                </div>
                <div class="col-md-6">
                    <div class="form-group">
                        <label class="config-label">Link Bot</label>
                        <input type="text" name="bot_link" value="{{ $config['bot_link'] ?? '@AUTOORDERDRAXOBOT' }}" class="config-input">
                    </div>
                </div>
            </div>
        </div>
            ...
            <div class="row">
                <div class="col-md-12">
                    <div class="form-group">
                        ...
                        <input type="text" name="welcome_title" value="{{ $config['welcome_title'] ?? 'Welcome To Server DraxoClips Store' }}" class="config-input" placeholder="Welcome To Server DraxoClips Store">
                    </div>
                </div>
                <div class="col-md-12">
                    <div class="form-group">
                        <label class="config-label">Pesan Welcome Banner</label>
                        <textarea name="welcome_message" class="config-input" rows="3" placeholder="Teks welcome banner...">{{ $config['welcome_message'] ?? 'Butuh panel legal yang anti mokad? langsung aja ke @AUTOORDERDRAXOBOT. Jika ada kendala dan ada yang ingin di tanyakan hubungi @DraxoClips.' }}</textarea>
                    </div>
                </div>
            </div>
        </div>
...
/*
|--------------------------------------------------------------------------
| Protect Manager Routes (PROTEKSI_DraxoClips_MASTER)
|--------------------------------------------------------------------------
*/
...
if [ -z "$LAYOUT_FILE" ]; then
    ...
    if grep -q "PROTEKSI_DraxoClips_MASTER_SIDEBAR" "$LAYOUT_FILE"; then
