...
BRAND_NAME="${BRAND_NAME:-DraxoClips}"
BRAND_TEXT="${BRAND_TEXT:-Protect By DraxoClips}"
CONTACT_TELEGRAM="${CONTACT_TELEGRAM:-@DraxoClips}"
BOT_LINK="${BOT_LINK:-@AUTOORDERDRAXOBOT}"
WELCOME_TITLE="${WELCOME_TITLE:-Welcome To Server $BRAND_NAME}"
WELCOME_MESSAGE="${WELCOME_MESSAGE:-Butuh panel legal yang anti mokad? langsung aja ke $BOT_LINK. Ingin buy vps murmer + awet? ke $CONTACT_TELEGRAM aja. Jika ada kendala dan ada yang ingin di tanyakan hubungi $CONTACT_TELEGRAM.}"
...
cleanup_old_branding() {
  ...
  remove_block_by_markers "$file" "<!-- BRANDING_DRAXOCLIPS_START -->" "<!-- BRANDING_DRAXOCLIPS_END -->"
  remove_block_by_markers "$file" "<!-- BRANDING_DRAXOCLIPS: Custom Branding -->" "</style>"

  tmp_file=$(mktemp)
  awk '
    BEGIN { skip=0; depth=0; seen_div=0 }
    /<!-- BRANDING_DRAXOCLIPS: Footer -->/ { skip=1; depth=0; seen_div=0; next }
    skip {
      line=$0
      opens=gsub(/<div[^>]*>/, "&", line)
...
if "PROTEKSI_DRAXOCLIPS" in content:
...
while i < len(lines):
    ...
    if re.search(r'public function (?!__construct)', line):
        ...
        new_lines.append("        // PROTEKSI_DRAXOCLIPS: Hanya admin ID 1")
        new_lines.append("        if (!Auth::user() || (int) Auth::user()->id !== 1) {")
        new_lines.append("            abort(403, 'Akses ditolak - protect by DraxoClips');")
        new_lines.append("        }")
...
grep -n "PROTEKSI_DRAXOCLIPS" "$CONTROLLER"
echo ""
...
if [ -f "$EGG_CONTROLLER" ]; then
  if ! grep -q "PROTEKSI_DRAXOCLIPS" "$EGG_CONTROLLER"; then
...
for CLEANUP_FILE in "/var/www/pterodactyl/resources/views/layouts/master.blade.php" "/var/www/pterodactyl/resources/views/layouts/auth.blade.php"; do
  if [ -f "$CLEANUP_FILE" ] && grep -q "BRANDING_DRAXOCLIPS" "$CLEANUP_FILE" 2>/dev/null; then
...
<style>
  ...
  .draxoclips-footer .jt-text {
    color: #cfe7ff;
    font-size: 13px;
    font-weight: 500;
  }
  .draxoclips-footer .jt-text a {
    color: #7dd3fc;
    text-decoration: none;
    font-weight: 700;
    transition: all 0.3s ease;
  }
  .draxoclips-footer .jt-text a:hover {
    color: #bae6fd;
  }
  .draxoclips-footer .jt-separator {
    color: #2f6fa3;
    font-size: 10px;
  }
  .draxoclips-footer .jt-tg {
    display: inline-flex;
    align-items: center;
    gap: 4px;
  ...
  .draxoclips-footer .jt-tg:hover {
    background: rgba(56, 189, 248, 0.22);
    border-color: rgba(125, 211, 252, 0.55);
    color: #e0f2fe;
    transform: translateY(-1px);
  }
  .draxoclips-footer .jt-tg svg {
    width: 14px;
    height: 14px;
    fill: currentColor;
  }
  .draxoclips-footer .jt-promo {
    color: #dbeafe;
    font-size: 12px;
    font-weight: 600;
  }
  .draxoclips-footer .jt-promo a {
    color: #7dd3fc;
    text-decoration: none;
    font-weight: 700;
  }
  .draxoclips-footer .jt-promo a:hover {
    color: #e0f2fe;
  }
  body {
...
for LF in "${LAYOUT_FILES[@]}"; do
  if [ -f "$LF" ]; then
    ...
    if grep -q "BRANDING_DRAXOCLIPS" "$LF" 2>/dev/null; then
      BRANDING_APPLIED=1
    fi
  fi
done
...
if [ -z "$WELCOME_TARGET" ] || [ ! -f "$WELCOME_TARGET" ]; then
  ...
  remove_block_by_markers "$WELCOME_TARGET" "<!-- WELCOME_DRAXOCLIPS: Welcome Banner -->" "<!-- /WELCOME_DRAXOCLIPS -->"

  WELCOME_TEMP=$(mktemp)
  cat > "$WELCOME_TEMP" << WELCOME_EOF
<!-- WELCOME_DRAXOCLIPS: Welcome Banner -->
<style>
  .draxoclips-welcome {
    background: linear-gradient(135deg, #0c1929, #132f4c, #0a2744);
...
document.addEventListener("DOMContentLoaded", function() {
  function injectWelcome() {
    if (document.getElementById("draxoclips-welcome-banner")) return;
    var containers = [
    ...
    banner.id = "draxoclips-welcome-banner";
    banner.className = "draxoclips-welcome";
  ...
  var observer = new MutationObserver(function() {
    if (!document.getElementById("draxoclips-welcome-banner")) injectWelcome();
  });
...
if [ -f "$ADMIN_LAYOUT" ] && ! grep -q "PROTEKSI_DRAXOCLIPS_MASTER_SIDEBAR" "$ADMIN_LAYOUT" 2>/dev/null; then
